...
def interact_with_app(helper: AppiumHelper, screenshots: dict):
    ...
    log_stdout("Appium - Wait until checkout button is visible.")
    helper.wait_until_clickable(by=AppiumBy.ID, value="com.corellium.cafe:id/tvCheckout")
...
def run_app_automation(config: AppiumConfig, udid: str):
    ...
    options.set_capability('appium:appActivity', config.target_app['cart_activity'])
