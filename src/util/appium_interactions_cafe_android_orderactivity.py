...
class AppiumHelper:
    ...
    def save_screenshot(self, filename: str = "screenshot.png"):
        ...
        save_screenshot_sleep_seconds: int = 5
        log_stdout(f"Pausing for {save_screenshot_sleep_seconds} seconds to stabilize WebRTC stream.")
        time.sleep(save_screenshot_sleep_seconds)
...
def interact_with_app(helper: AppiumHelper, screenshots: dict):
    ...
    log_stdout("Appium - Add the first drink option to cart.")
    ...
    log_stdout("Appium - Add the second drink option to cart.")
    helper.click_when_ready(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().resourceId(\"com.corellium.cafe:id/ivdrink\").instance(1)")
    helper.click_when_ready(by=AppiumBy.ID, value="com.corellium.cafe:id/fbAdd")

    log_stdout("Appium - Add the third drink option to cart.")
    helper.click_when_ready(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().resourceId(\"com.corellium.cafe:id/ivdrink\").instance(2)")
    helper.click_when_ready(by=AppiumBy.ID, value="com.corellium.cafe:id/fbAdd")

    log_stdout("Appium - Add the fourth drink option to cart.")
    helper.click_when_ready(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().resourceId(\"com.corellium.cafe:id/ivdrink\").instance(3)")
    helper.click_when_ready(by=AppiumBy.ID, value="com.corellium.cafe:id/fbAdd")

    log_stdout("Appium - Add the fifth drink option to cart.")
    helper.click_when_ready(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().resourceId(\"com.corellium.cafe:id/ivdrink\").instance(4)")
    helper.click_when_ready(by=AppiumBy.ID, value="com.corellium.cafe:id/fbAdd")

    log_stdout("Appium - Open cart and take screenshot.")
    helper.click_when_ready(by=AppiumBy.ID, value="com.corellium.cafe:id/abmCart")
    helper.save_screenshot(filename=f"{screenshots['cart']}_{int(datetime.now(timezone.utc).timestamp() * 1000)}.png")
...
def run_app_automation(config: AppiumConfig, udid: str):
    ...
    options.set_capability('appium:appActivity', config.target_app['order_activity'])
