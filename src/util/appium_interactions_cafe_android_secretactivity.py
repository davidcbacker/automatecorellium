...
def interact_with_app(helper: AppiumHelper, screenshots: dict):
    ...
    log_stdout("Waiting for secret activity page to load")
    helper.wait_until_clickable(by=AppiumBy.ID, value="com.corellium.cafe:id/bvDeleteDb")
...
def run_app_automation(config: AppiumConfig, udid: str):
    ...
    options.set_capability('appium:appActivity', config.target_app['secret_activity'])
