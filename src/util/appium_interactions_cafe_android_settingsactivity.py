...
def interact_with_app(helper: AppiumHelper, screenshots: dict):
    ...
    log_stdout("Appium - Wait until settings screen loads.")
    helper.wait_until_visible(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().text(\"Settings Screen\")")
...
def run_app_automation(config: AppiumConfig, udid: str):
    ...
    options.set_capability('appium:appActivity', config.target_app['settings_activity'])
