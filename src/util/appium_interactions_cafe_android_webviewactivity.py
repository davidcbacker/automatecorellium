...
def interact_with_app(helper: AppiumHelper, screenshots: dict):
    ...
    log_stdout("Appium - Wait until webview home screen loads.")
    helper.wait_until_visible(by=AppiumBy.ID, value="com.corellium.cafe:id/action_bar")
    helper.wait_until_visible(by=AppiumBy.ID, value="com.corellium.cafe:id/webviewHome")
...
def run_app_automation(config: AppiumConfig, udid: str):
    ...
    options.set_capability('appium:appActivity', config.target_app['web_view_activity'])
