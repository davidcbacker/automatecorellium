...
def run_app_automation(udid: str):
    ...
    options.set_capability('appium:appPackage', 'com.corellium.Cafe')
...
if __name__ == "__main__":
    match len(sys.argv):
        case 2:
            TARGET_DEVICE_UDID = sys.argv[1]
            log_stdout(f'Using Corellium virtual device with UDID {TARGET_DEVICE_UDID}.')
        case _:
            print('ERROR: Please pass in the UDID for the Corellium iOS/iPadOS device.', file=sys.stderr)
            sys.exit(1)

    run_app_automation(TARGET_DEVICE_UDID)
